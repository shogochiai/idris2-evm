...
executeOp EXP vm = binaryOp expOp vm where
  ...
  expOp base exp = fromInteger (pow (toInteger base) (cast $ toInteger exp))
...
returnOp vm =
  case popN 2 vm.stack of
    Ok ([offset, size], _) =>
      let retData = readBytes (cast $ toInteger offset) (cast $ toInteger size) (toByteList vm.memory)
...
executeHex codeHex calldataHex gasLimit =
  case (fromHex codeHex, fromHex calldataHex) of
